...
# 首页
def page_home():
    # 浅蓝色渐变banner - 使用内联样式确保一定显示
    st.markdown("""
    <div style='
        background: linear-gradient(135deg, #a8d8ea 0%, #c5e8f7 50%, #d4effc 100%);
        border-radius: 20px;
        padding: 30px 30px;
        margin-bottom: 25px;
        text-align: center;
    ...
    <div style='
        ...
        margin-bottom: 28px;
    '>
        <div style='margin-bottom:8px;'>{announce}</div>
        <div style='
            margin-top: 10px;
            width: 100%;
            overflow: hidden;
            ...
            padding: 0 10px;
        '>
            <marquee behavior='scroll' direction='left' scrollamount='5' style='color:#222222;font-weight:500;'>
                {stats_text}
            </marquee>
        </div>
    </div>
    ...
    with col1:
        kw = st.text_input("🔍 搜索物品/地点", placeholder="例如：校园卡、图书馆", key="home_search")
    with col2:
        cat = st.selectbox("📦 分类", ["全部"] + list(ITEM_ICONS.keys()), key="home_cat")
    ...
    with col_lost:
        ...
        <div style='
            ...
            overflow-y: auto;
        '>
            <h3 style='color:#d13434;margin-top:0;'>🔴 失物专区</h3>
        """, unsafe_allow_html=True)
    ...
    with col_found:
        ...
        <div style='
            ...
            overflow-y: auto;
        '>
            <h3 style='color:#1d8f45;margin-top:0;'>🟢 招领专区</h3>
        """, unsafe_allow_html=True)
...
def page_login_register():
    ...
    with t2:
        ...
        if st.button("注册", use_container_width=True):
            if not (sid and name and pwd):
                st.error("学号、姓名、密码不能为空")
                return
            try:
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
    ...
    with st.form("apply"):
        ...
        if st.form_submit_button("提交申请", use_container_width=True):
            if name and sid and phone:
                ...
                c = conn.cursor()
                c.execute(
                    "INSERT INTO apply_records (item_id,student_id,name,phone,note,created_at) VALUES (?,?,?,?,?,?)",
    ...
    with st.form("post_lost"):
        ...
        if st.form_submit_button("提交发布", use_container_width=True):
            if title and loc:
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?)''',
                          (st.session_state.user_id, typ, title, desc, loc, path, "lost", "pending", phone, wechat,
                           datetime.now()))
                conn.commit()
                conn.close()
    ...
    with st.form("post_found"):
        ...
        if st.form_submit_button("提交发布", use_container_width=True):
            if title and loc:
                ...
                (user_id,type,title,description,location,image_path,post_type,audit_status,contact_phone,contact_wechat,created_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?)''',
                          (st.session_state.user_id, typ, title, desc, loc, path, "found", "pending", phone, wechat,
                           datetime.now()))
                conn.commit()
                conn.close()
                st.success("提交成功，待管理员审核")
...
def page_profile():
    ...
    st.title("👤 个人中心")
    conn = sqlite3.connect(DB_PATH)
    u = pd.read_sql("SELECT name,student_id,phone,email FROM users WHERE id=?", conn,
                    params=(st.session_state.user_id,)).iloc[0]
